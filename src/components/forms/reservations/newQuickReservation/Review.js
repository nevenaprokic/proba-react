...
export default function Review({ offer, additionalServicesInputList }) {
  ...
  if (
    additionalServicesInputList.length != 1 &&
  ...
  let endDateReservation = addDays(
    new Date(offer.startDateReservation),
    offer.daysReservation
  );

  let startDateActionString =
    startDateAction.getDate() +
    "/" +
    (startDateAction.getMonth() + 1) +
    "/" +
    startDateAction.getFullYear();
  let endDateActionString =
    endDateAction.getDate() +
    "/" +
    (endDateAction.getMonth() + 1) +
    "/" +
    endDateAction.getFullYear();
  let startDateReservationString =
    startDateReservation.getDate() +
    "/" +
    (startDateReservation.getMonth() + 1) +
    "/" +
    startDateReservation.getFullYear();
  let endDateReservationString =
    endDateReservation.getDate() +
    "/" +
    (endDateReservation.getMonth() + 1) +
    "/" +
    endDateReservation.getFullYear();
      ...
      <List disablePadding>
        <ListItem key={"Offer price"} sx={{ py: 1, px: 0 }}>
          ...
          <Typography variant="body2">
            {offer.daysReservation + "x" + offer.price + "€"}
          </Typography>
        </ListItem>
        {additionalServicesInputList.length != 0 &&
        additionalServicesInputList[0].servicePrice !== "" ? (
          additionalServicesInputList.map((product) => (
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={7}>
          ...
          <Typography gutterBottom>
            {"Number of guests: " + offer.peopleNum}
          </Typography>
        </Grid>
