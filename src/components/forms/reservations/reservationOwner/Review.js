...
export default function Review({ reservation, checked }) {
  let totalPrice =
    parseInt(reservation.price) * parseInt(reservation.daysReservation);
  if (checked.length != 0) {
    checked.map((additional) => {
  ...
  let endDateReservation = addDays(
    new Date(reservation.startDateReservation),
      ...
      <List disablePadding>
        <ListItem key={"Offer price"} sx={{ py: 1, px: 0 }}>
          ...
          <Typography variant="body2">
            {reservation.daysReservation + "x" + reservation.price + "€"}
          </Typography>
        </ListItem>
        {checked.length != 0 ? (
          checked.map((product) => (
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={3}>
          ...
          <Typography gutterBottom>
            {"Name: " + reservation.offerName}
          </Typography>
          <Typography gutterBottom>
            {"Number of guests: " + reservation.peopleNum}
          </Typography>
        </Grid>
        <Grid item xs={12} sm={4}>
          ...
          <Typography gutterBottom>
            {"User name: " + reservation.clientUserName}
          </Typography>
        </Grid>
