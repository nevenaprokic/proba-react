...
function Row({deleteRequest, setDeleteRequests, deleteRequests}) {
    ...
    return (
         !!deleteRequest && !!setDeleteRequests &&
      ...
      <React.Fragment>
        <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
          ...
          <TableCell sx={{fontSize: 16}} align="center" >{deleteRequest.role}</TableCell>
          <TableCell sx={{fontSize: 16}} align="center">{deleteRequest.userFirstName}</TableCell>
          <TableCell sx={{fontSize: 16}} align="center">{deleteRequest.userLastName}</TableCell>
          <TableCell ><Button 
                    type="submit"
                    variant="contained"
                    sx={{float:"right"}}
    ...
    return (
        ...
        <TableContainer component={Paper} className="tableContainer">
            <Table aria-label="collapsible table" >
                ...
                {deleteRequests.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
                    
                    <Row key={row.id} deleteRequest={row} setDeleteRequests={setDeleteRequests} deleteRequests={deleteRequests}/>
                ))}
                </TableBody>
            </Table>
        </TableContainer>
      ...
      />
      {deleteRequests.length === 0 && <Typography variant="h6" sx={{color:"#CC7351"}}>There are currently no reservation reports for reviewing</Typography>}
    </div>
    );
}
