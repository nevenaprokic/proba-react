...
function LoyaltyCard({ category, type, setLoyaltyCategories }) {
  ...
  if (!!category && !!type && !!setLoyaltyCategories) {
    return (
      <Card sx={{ border: "solid thin #ADC2A9" }}>
        <CardHeader
          avatar={
            <Avatar src={categoryIcon} sx={{ width: 56, height: 56 }}></Avatar>
          }
          title={category.name}
          sx={{ fontSize: "35px", color: { color } }}
          subheader={
            type == userType.CLIENT ? "Client category" : "Owner category"
          }
        />
        ...
        >
          <ChangeLoyaltyCategory
            close={handleCloseForm}
            loyaltyCategory={category}
            categoryType={type}
            setLoyaltyCategories={setLoyaltyCategories}
          />
        </Modal>
        <Modal
          open={openDialog}
